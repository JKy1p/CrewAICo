...
class CompanyResearchCrew:
    ...
    def setup_crew(self, target_account: str, topics: list[str]):
        ...
        research_manager = agents.research_reviewer(
            target_account, topics)
        company_research_agent = agents.account_researcher()

        company_research_tasks = [
            tasks.company_research(company_research_agent, company, topics)
            for company in target_account
        ]

        manage_research_task = tasks.manage_research(
            research_manager, target_account, topics, company_research_tasks)
